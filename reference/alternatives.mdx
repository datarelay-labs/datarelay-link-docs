...
| --- | --- | --- | --- | --- |
| **DataRelay Link** | Self-hosted secure relay + enrollment/lifecycle + Internet Access | You | No, for Managed Hosts | A few to a few dozen systems where you want your own public server, predictable service ports, and explicit outbound policy |
| **Manual self-hosted reverse proxy** | Self-managed tunnel/reverse proxy configuration | You | Usually no for Managed Hosts | Engineers who want maximum low-level flexibility and are comfortable maintaining configuration directly |
...
| Zero-Touch Managed Host enrollment | **Built in** | Usually manual/custom | Managed agent/account onboarding | Managed connector/Zero Trust onboarding | Managed device/tailnet enrollment |
| Persistent Remote Service endpoint reservation | **Built in** | Operator-managed | Provider-managed endpoint addressing | Hostname/route oriented | Tailnet IP/DNS oriented |
| Small-fleet Managed Host and Remote Service lifecycle | **Built in** | Operator-managed | Managed SaaS model | Managed Zero Trust model | Managed tailnet/device model |
| Internet Access (controlled HTTP/HTTPS egress) | **Product direction; release qualification in progress** | Requires separate proxy/policy design | Not the primary product model | Gateway/Zero Trust options available | Not the primary product model |
| SaaS account required for normal operation | **No** | **No** | **Yes** | **Yes** | **Yes** |
| Intended scale of this project | **A few to a few dozen Managed Hosts** | Depends on your operations | Vendor-managed | Vendor-managed | Vendor-managed |

<Note>
  DataRelay Link should not be presented as a replacement for enterprise ZTNA, SASE, endpoint compliance, DLP, or a full VPN/mesh platform. Internet Access v2.4 is deliberately narrower than a Secure Web Gateway.
</Note>
...
- an Agent Host may expose multiple local or reachable LAN targets through Remote Services
- you want Zero-Touch enrollment, Managed Host identity, persistent Remote Service endpoints, lifecycle controls, diagnostics, and backup/restore
...
| --- | --- |
| Self-hosted public relay, small fleet, direct SSH/TCP, Internet Access | **DataRelay Link** |
